...
describe('resolveStyle', () => {
  it('matches type selector', () => {
    ...
    expect(resolveStyle(sheet, folderNode)).toEqual({ icon: 'url(folder.svg)' })
  })
  ...
  it('matches [name="..."] selector', () => {
    ...
    expect(resolveStyle(sheet, dockerfile)).toEqual({ icon: 'url(docker.svg)' })
  ...
  it('matches pseudo-class :expanded', () => {
    ...
    const expanded = node({ name: 'src', type: 'folder', stateFlags: StateFlags.Expanded })
    const collapsed = node({ name: 'src', type: 'folder', stateFlags: StateFlags.None })
  ...
  it('matches descendant combinator', () => {
    ...
    const unrelated = node({ name: 'workflows', type: 'folder', path: '/workflows' })
  ...
  it('applies specificity: name > extension > type', () => {
    ...
    expect(resolveStyle(sheet, tsconfig)).toEqual({ icon: 'url(tsconfig.svg)' })
    expect(resolveStyle(sheet, regular)).toEqual({ icon: 'url(ts.svg)' })
  ...
  it('matches meta attributes', () => {
    ...
    const modified = node({ name: 'test.ts', meta: { 'vcs-status': 'modified' } })
