...
describe('createFsNode', () => {
  it('computes baseExt and fullExt from simple filename', () => {
    const n = createFsNode({ type: 'file', name: 'index.ts', path: '/index.ts' })
    expect(n.baseExt).toBe('ts')
    expect(n.fullExt).toBe('ts')
  })

  it('computes compound extension correctly', () => {
    const n = createFsNode({ type: 'file', name: 'utils.test.ts', path: '/utils.test.ts' })
    expect(n.baseExt).toBe('ts')
  ...
  it('returns empty extensions for folder', () => {
    const n = createFsNode({ type: 'folder', name: 'src.backup', path: '/src.backup' })
    expect(n.baseExt).toBe('')
    expect(n.fullExt).toBe('')
  })

  it('returns empty extensions for dotfile without extension', () => {
    const n = createFsNode({ type: 'file', name: '.gitignore', path: '/.gitignore' })
  ...
  it('handles extensionless file', () => {
    const n = createFsNode({ type: 'file', name: 'Makefile', path: '/Makefile' })
    expect(n.baseExt).toBe('')
    expect(n.fullExt).toBe('')
  })
  ...
  it('preserves parent reference', () => {
    ...
    const child = createFsNode({ type: 'file', name: 'f.ts', path: '/f.ts', parent })
  ...
  it('preserves provided lang', () => {
    const n = createFsNode({ type: 'file', name: 'f.ts', path: '/f.ts', lang: 'typescript' })
