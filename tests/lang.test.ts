...
describe('[lang="..."] selector', () => {
  it('matches by language', () => {
    ...
    const tsFile = node({ name: 'index.ts', path: '/index.ts', lang: 'typescript' })
    const pyFile = node({ name: 'app.py', path: '/app.py', lang: 'python' })
    const mdFile = node({ name: 'README.md', path: '/README.md', lang: 'markdown' })

    expect(resolveStyle(sheet, tsFile)).toEqual({ icon: 'url(ts.svg)', color: 'blue' })
    expect(resolveStyle(sheet, pyFile)).toEqual({ icon: 'url(py.svg)', color: 'green' })
  ...
  it('combines lang with other selectors', () => {
    ...
    const testFile = node({ name: 'app.test.ts', path: '/app.test.ts', lang: 'typescript' })
    const srcFile = node({ name: 'app.ts', path: '/app.ts', lang: 'typescript' })

    expect(resolveStyle(sheet, testFile)).toEqual({ icon: 'url(ts.svg)', badge: 'test' })
  ...
  it('lang works with themes', () => {
    ...
    expect(resolveStyle(sheet, tsFile)).toEqual({ icon: 'url(ts.svg)', color: 'white' })
    expect(resolveStyle(sheet, tsFile, 'dark')).toEqual({ icon: 'url(ts.svg)', color: '#58a6ff' })
    expect(resolveStyle(sheet, tsFile, 'light')).toEqual({ icon: 'url(ts.svg)', color: '#0366d6' })
  })
  ...
  it('lang with descendant combinator', () => {
    ...
    const orphanTs = node({ name: 'index.ts', path: '/index.ts', lang: 'typescript' })
