...
describe('integration', () => {
  it('full stylesheet with all features', () => {
    ...
    // Modified file
    const modified = node({ name: 'dirty.ts', meta: { 'vcs-status': 'modified', inVcsRepo: true } })
  ...
  it('handles deep nested ancestor matching', () => {
    ...
    const packages = node({ name: 'packages', type: 'folder', path: '/packages' })
    const core = node({ name: 'core', type: 'folder', path: '/packages/core', parent: packages })
    const src = node({ name: 'src', type: 'folder', path: '/packages/core/src', parent: core })
    const file = node({ name: 'index.ts', path: '/packages/core/src/index.ts', parent: src })
  ...
  it('mixed themes + layers + nesting', () => {
    ...
    const folder = node({ name: 'src', type: 'folder', stateFlags: StateFlags.Expanded })
    ...
    expect(resolveStyle(sheet, folder)).toEqual({ icon: 'url(folder-open.svg)' })
  })
