...
describe('LayeredResolver', () => {
  ...
  it('nested layer overrides global', () => {
    ...
    `, '/project/', LayerPriority.PROJECT))

    const inProject = node({ name: 'index.ts', path: '/project/index.ts' })
    const outside = node({ name: 'index.ts', path: '/other/index.ts' })

    expect(resolver.resolveStyle(inProject)).toEqual({ icon: 'url(project-ts.svg)' })
    expect(resolver.resolveStyle(outside)).toEqual({ icon: 'url(global-ts.svg)' })
  })
  ...
  it('deeper nested layer overrides parent layer', () => {
    ...
    `, '/project/sub/', LayerPriority.nestedPriority(1)))
  ...
  it('merges properties across layers', () => {
    ...
    expect(resolver.resolveStyle(n)).toEqual({ color: 'white', icon: 'url(ts.svg)' })
  })
  ...
  it('setLayers replaces all layers', () => {
    ...
    resolver.setLayers([
      createLayer(`file { icon: url(new.svg); }`, '/', LayerPriority.GLOBAL),
    ])
