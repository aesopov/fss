...
describe('nested rules (&)', () => {
  it('basic nesting with &:pseudo-class', () => {
    ...
    const expanded = node({ name: 'src', path: '/src', type: 'folder', stateFlags: StateFlags.Expanded })

    expect(resolveStyle(sheet, collapsed)).toEqual({ icon: 'url(folder.svg)' })
    expect(resolveStyle(sheet, expanded)).toEqual({ icon: 'url(folder-open.svg)' })
  })
  ...
  it('deep nesting (two levels)', () => {
    ...
    const srcExpanded = node({ name: 'src', path: '/src', type: 'folder', stateFlags: StateFlags.Expanded })
    const plainFolder = node({ name: 'docs', path: '/docs', type: 'folder' })

    expect(resolveStyle(sheet, plainFolder)).toEqual({ icon: 'url(folder.svg)' })
    expect(resolveStyle(sheet, srcCollapsed)).toEqual({ icon: 'url(code-folder.svg)' })
    expect(resolveStyle(sheet, srcExpanded)).toEqual({ icon: 'url(code-folder-open.svg)' })
  })
  ...
  it('comma-separated parent × nested child', () => {
    ...
    const expandedFolder = node({ name: 'src', path: '/src', type: 'folder', stateFlags: StateFlags.Expanded })
    expect(resolveStyle(sheet, expandedFolder)).toEqual({ icon: 'url(open.svg)' })
  })
  ...
  it('nesting inside @theme', () => {
    ...
    expect(resolveStyle(sheet, collapsed, 'dark')).toEqual({ icon: 'url(folder-dark.svg)' })
    expect(resolveStyle(sheet, expanded, 'dark')).toEqual({ icon: 'url(folder-open-dark.svg)' })
  })
  ...
  it('nesting inside @sorting', () => {
    ...
    const nodeModules = node({ name: 'node_modules', path: '/node_modules', type: 'folder' })
