...
describe('parseStylesheet', () => {
  ...
  it('parses attribute selectors', () => {
    ...
    expect(sel.target.attrs[0]).toEqual({ name: 'ext', operator: '=', value: 'ts' })
  })
  ...
  it('parses descendant combinator', () => {
    ...
    const sel = sheet.styleRules[0].selectors[0]
    expect(sel.target.attrs[0]).toEqual({ name: 'name', operator: '=', value: 'workflows' })
    expect(sel.ancestors.length).toBe(1)
    expect(sel.ancestors[0].attrs[0]).toEqual({ name: 'name', operator: '=', value: '.github' })
  })
  ...
  it('parses boolean attribute selector', () => {
    ...
    const sel = sheet.styleRules[0].selectors[0]
    expect(sel.target.attrs[0]).toEqual({ name: 'inVcsRepo', operator: null, value: null })
  })
  ...
  it('rules are sorted by specificity then order', () => {
    ...
    for (let i = 1; i < sheet.styleRules.length; i++) {
      ...
      expect(
        curr.specificity > prev.specificity
        || (curr.specificity === prev.specificity && curr.order > prev.order),
      ).toBe(true)
    }
  })
})
